...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ==========================================
    // 0.5. Gold-foil scratch card that reveals the date
    // ==========================================
    let revealAllScratch = () => {};
    let scratchInitDone = false;
    ...
    function initScratchCard() {
        if (scratchInitDone) return;
        const card = document.getElementById('scratchCard');
        const canvas = document.getElementById('scratchCanvas');
        const wrap = document.getElementById('scratchCardContainer');
        if (!card || !canvas) return;
        scratchInitDone = true;

        const ctx = canvas.getContext('2d');
        let W = 0, H = 0, dpr = 1;
        let started = false, done = false, drawing = false, last = null;
        let lastCheck = 0, lastFleck = 0, lastBuzz = 0;

        // Paint the metallic foil: gradient, brushed grain, embossed jaali lattice, a plaque and the lettering
        function paintFoil() {
            W = Math.max(1, Math.round(card.clientWidth));
            H = Math.max(1, Math.round(card.clientHeight));
            ...
            [[0, '#8C6A2B'], [0.18, '#D6AF55'], [0.36, '#F6E2A2'], [0.52, '#C9993F'],
             [0.7, '#F1D58A'], [0.86, '#B98A36'], [1, '#8C6A2B']].forEach(([o, c]) => g.addColorStop(o, c));
            ctx.fillStyle = g;
            ctx.fillRect(0, 0, W, H);

            for (let i = 0; i < (W * H) / 16; i++) {
                ctx.fillStyle = Math.random() < 0.5 ? 'rgba(255, 248, 220, 0.12)' : 'rgba(90, 60, 15, 0.09)';
                ctx.fillRect(Math.random() * W, Math.random() * H, Math.random() * 2.5 + 0.5, 0.6);
            }
            ...
            for (let y = 0, row = 0; y < H + step; y += step, row++) {
                for (let x = (row % 2) * step / 2; x < W + step; x += step) {
                    diamond(x + 0.7, y + 0.7, 5); ctx.strokeStyle = 'rgba(255, 245, 210, 0.35)'; ctx.stroke();
                    diamond(x, y, 5); ctx.strokeStyle = 'rgba(110, 75, 20, 0.28)'; ctx.stroke();
                }
            }

            const cx = W / 2, cy = H / 2 + 6;
            const plaque = ctx.createRadialGradient(cx, cy, 4, cx, cy, W * 0.4);
            plaque.addColorStop(0, 'rgba(255, 243, 205, 0.85)');
            plaque.addColorStop(1, 'rgba(255, 243, 205, 0.25)');
            ctx.beginPath();
            ctx.ellipse(cx, cy, W * 0.37, H * 0.31, 0, 0, Math.PI * 2);
            ctx.fillStyle = plaque;
            ctx.fill();
            ctx.lineWidth = 1;
            ctx.strokeStyle = 'rgba(110, 75, 20, 0.45)';
            ctx.stroke();
            ctx.beginPath();
            ctx.ellipse(cx, cy, W * 0.37 - 4, H * 0.31 - 4, 0, 0, Math.PI * 2);
            ctx.strokeStyle = 'rgba(255, 245, 210, 0.6)';
            ctx.stroke();

            ctx.textAlign = 'center';
            ctx.textBaseline = 'middle';
            ctx.font = `400 ${Math.round(H * 0.21)}px "Pinyon Script", "Playfair Display", cursive`;
            ctx.fillStyle = 'rgba(255, 245, 210, 0.7)';
            ctx.fillText('Save the Date', cx + 1, cy - H * 0.05 + 1);
            ctx.fillStyle = '#6E1320';
            ctx.fillText('Save the Date', cx, cy - H * 0.05);
            ctx.font = `600 ${Math.max(9, Math.round(H * 0.064))}px Montserrat, Helvetica, sans-serif`;
            if ('letterSpacing' in ctx) ctx.letterSpacing = '3px';
            ctx.fillStyle = 'rgba(80, 48, 12, 0.85)';
            ctx.fillText('SCRATCH TO REVEAL', cx, cy + H * 0.16);
            if ('letterSpacing' in ctx) ctx.letterSpacing = '0px';
        }

        paintFoil();
        if (document.fonts && document.fonts.ready) {
            document.fonts.ready.then(() => { if (!started && !done) paintFoil(); });
        }
        window.addEventListener('resize', () => { if (!started && !done) paintFoil(); });
        ...
        function scratchTo(p) {
            ...
            ctx.lineWidth = 40;
            ctx.beginPath();
            ctx.moveTo(last ? last.x : p.x - 0.1, last ? last.y : p.y);
            ctx.lineTo(p.x, p.y);
            ctx.stroke();
            last = p;
        }
        ...
        function finish() {
            ...
            setTimeout(() => { canvas.style.display = 'none'; }, 750);
            onScratchRevealed();
        }
        ...
        canvas.addEventListener('pointerdown', (e) => {
            ...
            if (!started) { started = true; card.classList.add('scratching'); }
            last = null;
            scratchTo(pointAt(e));
        });
        canvas.addEventListener('pointermove', (e) => {
            ...
            if (now - lastFleck > 35) { fleck(p); lastFleck = now; }
            if (navigator.vibrate && now - lastBuzz > 90) {
                try { navigator.vibrate(5); } catch (err) { /* ignore */ }
                lastBuzz = now;
            }
            if (now - lastCheck > 220) {
                lastCheck = now;
                if (coverage() > 0.5) finish();
            }
        });
        const endStroke = () => {
            if (!drawing) return;
